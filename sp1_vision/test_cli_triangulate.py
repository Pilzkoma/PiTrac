#!/usr/bin/env python3
"""Unit tests for cli_triangulate module - resume and corruption handling."""
...
class TestManifestHandling(unittest.TestCase):
    ...
    def test_manifest_and_disk_agreement(self):
        """Test when manifest and disk agree."""
        test_dir = tempfile.mkdtemp()
        try:
    ...
    def test_manifest_disk_divergence(self):
        """Test detection of manifest/disk divergence."""
        test_dir = tempfile.mkdtemp()
        try:
            cam1_dir = os.path.join(test_dir, "cam1")
            os.makedirs(cam1_dir)

            # Create 3 images (simulating crash)
            for num in [1, 2, 3]:
                with open(os.path.join(cam1_dir, "gs_{:02d}.png".format(num)), "w") as f:
                    f.write("x")

            # Create manifest with only 2 shots
            manifest = {
                "shots": [
                    {"name": "gs_01.png", "tape_mm": 350.0, "series": "depth"},
                    {"name": "gs_02.png", "tape_mm": 420.0, "series": "depth"}
            ...
            # Should use the max
            self.assertEqual(max(max_disk, max_manifest) + 1, 4)
        finally:
            shutil.rmtree(test_dir)
